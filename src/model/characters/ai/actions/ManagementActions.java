...
public class ManagementActions extends BaseActions {
    ...
    public static final Predicate<Person> hasInvestInGuildBuildings = person ->   person.hasAspiration(Aspiration.INVEST_IN_ALLOY_PRODUCTION)
    ...
    public static void tradeMarket(Person person) {
        ...
        if (fMarketRatio < foodRatio && amountGoldToSpend > 0){
            ...
            if(exchange.exchangeResources(amountGoldToSpend*2, Resource.Food, Resource.Gold, person.getCharacter())) {
                ...
                person.logAction("Trade Market",String.format("Bought %d food to balance the market since market has %1f ratio of food. Spent %d Gold", amountAfter , fMarketRatio, amountGoldToSpend*2));
        ...
        if (aMarketRatio < alloyRatio && amountGoldToSpend > 0){
            ...
            if(exchange.exchangeResources(amountGoldToSpend, Resource.Alloy, Resource.Gold, person.getCharacter())) {
                ...
                person.logAction("Trade Market",String.format("Bought %d alloys to balance the market since market has %2f ratio of alloys. Spent %d Gold", amountAfter, aMarketRatio, amountGoldToSpend));
                person.removeAspiration(Aspiration.TRADE_MARKET);
            }
        }
    }
    ...
    class EvaluateRoleSpecificNeeds extends WeightedObject {
        ...
        private void evaluateTaxPolicy(){
            if(!evaluateExtremeTaxPolicy()){
                if(!evaluateLowTaxPolicy()){
                    if(doesntHaveTaxAspirationYet()) {
...
//                    logAction("aggressive or disloyal", "Added aspiration to set extreme tax policy for having very high aggressive or disloyal personality");
    ...
    class TakeActionOnNeeds extends WeightedObject {
        int counter = 0; // makes sure the method doesn't get stuck
        ...
        public void defaultAction() {
            try {
                ...
                if (immediateNeedForEverything() && counter < 100) {
                    counter++;
                    return; // quick return if there is need for everything, let production ramp up
                }
                counter++;

                if(person.isPlayer() && person.getAspirations().contains(Aspiration.UPGRADE_PROPERTY)){
                    debugPoint();
                }

                if(person.getAspirations().isEmpty()){
                    person.addAspiration(Aspiration.DEPOSIT_TO_VAULT);
                }

                for (Aspiration aspiration : person.getAspirations()) {

                    if (aspiration.spendsResources()) {
                        if(!person.getPaymentManager().allowNPCToSpendMoney()) continue; // Check paymentManager
                    }

                    aspiration.handle(person);
                }
        ...
        private void handleSpareResources(Property property) {
            ...
            if (vd.isPositive()) {
                if (property.getVault().deposit(wallet, vd)) {
                    logAction("Deposited to Vault", String.format("%s deposited to the Vault", vd));
                    person.addAspiration(Aspiration.INCREASE_PROPERTY_DEFENCE);
                    person.removeAspiration(Aspiration.DEPOSIT_TO_VAULT); // this should be removed here
                }
            }
        }

        private boolean handleInvestInFoodProduction(UtilitySlot utilitySlot) {
            if (person.getAspirations().contains(Aspiration.SAVE_RESOURCES)) return true;
        ...
        private boolean handleInvestInAlloyProduction(UtilitySlot utilitySlot) {
            if (person.getAspirations().contains(Aspiration.SAVE_RESOURCES)) return true;
        ...
        private boolean handleInvestInGoldProduction(UtilitySlot utilitySlot) {
            if (hasInvestInGuildBuildings.test(person)) {
        ...
        private void handleGetAlloysInstantly() {
        ...
        private void handleGetFoodInstantly() {
        ...
        private void handleGetGoldInstantly() {
    ...
    class EvaluateNeeds extends WeightedObject {
        ...
        private void evaluatePropertyNeed() {
            if(Time.year <= 4) {
                ...
                propertyUpgrade += Time.year;
            }
            if(propertyUpgrade < 2*Time.year) return;
